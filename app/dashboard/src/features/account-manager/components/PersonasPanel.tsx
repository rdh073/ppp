...
import type { StreamEnvelope } from '../../../shared/http/client';
import { personaStatusColor as STATUS_COLOR, inputStyle, accentBtnStyle as btnStyle } from '../../../components/ui/statusStyles';
...
export function PersonasPanel() {
  const stream = useMemo(() => ({
    topics: ['account-manager.personas'],
    getKey: (persona: Persona) => persona.id,
    applyEvent: (current: Persona[], event: StreamEnvelope<Persona>) => {
      if (event.type === 'delete') {
        return current.filter((persona) => persona.id !== event.entityId);
      }
      const index = current.findIndex((persona) => persona.id === event.entityId);
      if (index === -1) {
  ...
  const { data: personas, loading, error, reload: load } = useDataList(listPersonas, { stream });
      ...
      <div style={{ border: '1px solid var(--border)', borderRadius: '0.5rem', overflow: 'hidden' }}>
        {personas.length === 0 && !loading ? (
          <div style={{ padding: '1rem', textAlign: 'center', color: 'var(--muted)', fontSize: '0.8rem' }}>
            No personas saved. Add one manually or let account creation generate one via AI.
          </div>
        ) : (
