...
function LoginPlatformPanel({ platform, label }: LoginPlatformPanelProps) {
  ...
  const stream = useMemo(() => ({
    topics: [`account-manager.logins.${platform}`],
    getKey: (run: LoginRun) => run.id,
  }), [platform]);
  const { data: runs, loading, error, reload } = useDataList(api.list, {
    stream,
  });
  ...
  return (
    <LoginList
      ...
      hasRunning={runs.some((c) => c.status === 'running')}
    />
  );
}
...
function LoginList({ runs, loading, error, emptyText, form, hasRunning }: LoginListProps) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontSize: '0.8rem', color: 'var(--muted)' }}>
          {runs.length} login{runs.length !== 1 ? 's' : ''}
          {hasRunning && (
      ...
      <div style={{ border: '1px solid var(--border)', borderRadius: '0.5rem', overflow: 'hidden' }}>
        {runs.length === 0 && !loading ? (
          ...
          runs.map((run) => <LoginRow key={run.id} run={run} />)
        )}
      </div>
    </div>
  );
