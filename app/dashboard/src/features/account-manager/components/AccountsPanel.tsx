...
export function AccountsPanel() {
  ...
  const stream = useMemo(() => ({
    topics: ['account-manager.accounts'],
    getKey: (account: Account) => account.id,
  }), []);
  const { data: accounts, loading, error, reload } = useDataList(() => listAccounts(), { stream });

  const google = accounts.filter((a) => a.kind === 'google');
  const instagram = accounts.filter((a) => a.kind === 'instagram');
  const visibleCount =
    kindFilter === 'google' ? google.length :
    kindFilter === 'instagram' ? instagram.length :
    accounts.length;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        <span style={{ fontSize: '0.8rem', color: 'var(--muted)' }}>
          {visibleCount} account{visibleCount !== 1 ? 's' : ''}
        </span>
      ...
      {!loading && visibleCount === 0 ? (
        <div style={{ padding: '1.5rem', textAlign: 'center', color: 'var(--muted)', fontSize: '0.8rem', border: '1px solid var(--border)', borderRadius: '0.5rem' }}>
          No accounts created yet. Start an account-creation run to create accounts.
        </div>
      ) : (
        <>
