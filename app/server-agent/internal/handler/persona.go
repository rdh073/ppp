...
// PersonaHandler serves GET/POST/DELETE /personas and GET/DELETE /personas/{id}.
type PersonaHandler struct {
	store store.PersonaStore
}

func NewPersonaHandler(s store.PersonaStore) *PersonaHandler {
	return &PersonaHandler{store: s}
}
...
func (h *PersonaHandler) list(w http.ResponseWriter, r *http.Request) {
	...
	items := h.store.List(kind, status)
	if items == nil {
		items = []domain.Persona{}
	}
	jsonOK(w, items)
}
