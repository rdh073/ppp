...
//
//	GET  /accounts                  — list accounts (filter: ?kind=google&deviceId=...)
//	POST /accounts/google           — register a Google account (called by Rhino script via http())
//	POST /accounts/instagram        — register an Instagram account (called by Rhino script via http())
...
func (h *AccountHandler) list(w http.ResponseWriter, r *http.Request) {
	...
	items := h.store.List(kind, deviceID)
	if items == nil {
...
// register handles POST /accounts/{kind}.
...
func (h *AccountHandler) register(w http.ResponseWriter, r *http.Request, kind string) {
	...
	jsonOK(w, map[string]string{"accountId": a.ID})
}
...
func NewAccountToolHandler(s store.AccountStore) *AccountToolHandler {
	return &AccountToolHandler{store: s}
}
...
func (h *AccountToolHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(accountToolInvokeResp{Result: map[string]string{"accountId": a.ID}})
}
