...
type PendingTaskAssigner interface {
	TryAssignPendingToDevice(ctx context.Context, deviceID domain.DeviceID) error
	OnDeviceOffline(ctx context.Context, deviceID domain.DeviceID) error
}
...
type AgentLifecycleUseCase struct {
	...
	assigner          PendingTaskAssigner // optional; nil-safe
...
// SetAssigner wires the pending-task assigner so that newly connected devices are
// automatically assigned pending tasks from the queue.
// This remains a setter because the assigner has a justified circular
// dependency: it depends on both TaskControl and AgentLifecycle.
func (u *AgentLifecycleUseCase) SetAssigner(a PendingTaskAssigner) {
	u.assigner = a
}
