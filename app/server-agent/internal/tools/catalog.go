	...
	return ToolDefinition{
		Manifest: ToolManifest{
			...
			Description: "Translates 1-indexed grid square numbers into absolute screen tap coordinates. " +
				"Assumes a square grid (rows = cols). Returns up to 9 tap slots (tap0..tap8); unused slots are empty strings.",
			Deterministic: true,
			Timeout:       250 * time.Millisecond,
			InputSchema: rawSchema(`{
				"type":"object",
			...
			OutputSchema: rawSchema(`{
				"type":"object",
				"required":["tap0","tap1","tap2","tap3","tap4","tap5","tap6","tap7","tap8","count"],
				"properties":{
