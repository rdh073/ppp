...
func newRecoveryLog() *slog.Logger {
...
func TestRuntimeRecovery_BootstrapsMissingWorkflowState(t *testing.T) {
	...
	uc := workflowruntime.NewRuntimeRecovery(tasks, states, newRecoveryLog())
...
func TestRuntimeRecovery_FileStoresAcrossReopen(t *testing.T) {
	...
	uc := workflowruntime.NewRuntimeRecovery(reopenedTasks, reopenedStates, newRecoveryLog())
	report, err := uc.Recover(ctx)
	if err != nil {
		t.Fatalf("Recover: %v", err)
	}
