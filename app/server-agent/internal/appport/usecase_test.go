...
func TestHello_RegistersSessionAndFiresOnlineEvent(t *testing.T) {
	...
	uc := devicectrl.NewAgentLifecycle(reg, proc, nil, nil, newLog())

	resp, err := uc.Hello(context.Background(), devicectrl.HelloRequest{
		DeviceID:        "dev-1",
...
func TestHello_ReplacesExistingSession(t *testing.T) {
	...
	// First hello.
	resp1, _ := uc.Hello(context.Background(), devicectrl.HelloRequest{DeviceID: "dev-2"}, noopSender{})
	// Second hello with same device.
	resp2, _ := uc.Hello(context.Background(), devicectrl.HelloRequest{DeviceID: "dev-2"}, noopSender{})
...
func TestResume_UnknownSession_Error(t *testing.T) {
	reg := registry.New()
	uc := devicectrl.NewAgentLifecycle(reg, &recordingProcessor{}, nil, nil, newLog())

	_, err := uc.Resume(context.Background(), devicectrl.ResumeRequest{
...
func TestResume_KnownSession_Accepted(t *testing.T) {
	...
	resp, _ := uc.Hello(context.Background(), devicectrl.HelloRequest{DeviceID: "dev-3"}, noopSender{})
	proc.events = nil // reset

	// Resume with the issued sessionID.
	resumeResp, err := uc.Resume(context.Background(), devicectrl.ResumeRequest{
...
func TestDisconnect_RemovesSessionAndFiresOfflineEvent(t *testing.T) {
	...
	resp, _ := uc.Hello(context.Background(), devicectrl.HelloRequest{DeviceID: "dev-4"}, noopSender{})
	proc.events = nil
...
func TestDisconnect_Idempotent_NoDuplicateOfflineEvent(t *testing.T) {
	...
	resp, _ := uc.Hello(context.Background(), devicectrl.HelloRequest{DeviceID: "dev-idempotent"}, noopSender{})
	proc.events = nil
...
func newTaskUC(reg registry.AgentRegistry) (*workflowruntime.TaskControlUseCase, *store.MemoryTaskStore) {
	tasks := store.NewMemoryTaskStore()
	states := store.NewMemoryWorkflowStateStore()
	proc := &recordingProcessor{}
	return workflowruntime.NewTaskControl(tasks, states, proc, reg, newLog()), tasks
}
...
func TestCreateTask_WithConnectedDevice_BootstrapsWorkflowStateWithInputArtifacts(t *testing.T) {
	...
	uc := workflowruntime.NewTaskControl(tasks, states, orch, reg, newLog())

	task, err := uc.CreateTask(ctx, workflowruntime.CreateTaskRequest{
		Goal:           "run on device",
...
func TestCreateTask_WithUnconnectedDevice_Error(t *testing.T) {
	...
	_, err := uc.CreateTask(context.Background(), workflowruntime.CreateTaskRequest{
		Goal:     "will fail",
...
func TestCreateTask_WithConnectedDevice_Busy_Error(t *testing.T) {
	...
	first, err := uc.CreateTask(context.Background(), workflowruntime.CreateTaskRequest{
		Goal:     "first task",
		DeviceID: "dev-busy",
	})
	if err != nil {
...
func TestCreateTask_EmptyGoal_Error(t *testing.T) {
	...
	_, err := uc.CreateTask(context.Background(), workflowruntime.CreateTaskRequest{Goal: ""})
	if err == nil {
...
func TestCancelTask_Running_Cancelled(t *testing.T) {
	...
	task, _ := uc.CreateTask(context.Background(), workflowruntime.CreateTaskRequest{Goal: "cancel me"})
...
func TestCancelTask_AlreadyTerminal_Error(t *testing.T) {
	...
	task, _ := uc.CreateTask(context.Background(), workflowruntime.CreateTaskRequest{Goal: "done task"})
...
func TestGetTask_Found(t *testing.T) {
	...
	task, _ := uc.CreateTask(context.Background(), workflowruntime.CreateTaskRequest{Goal: "get me"})
...
func TestListTasks_EnrichesWorkflowAndOutbox(t *testing.T) {
	...
	uc := workflowruntime.NewTaskControl(tasks, states, &recordingProcessor{}, reg, newLog())
	...
	items, err := uc.ListTasks(ctx, workflowruntime.ListTaskQuery{Limit: 10})
	if err != nil {
