...
- Phases 1, 2, and 2.5 are now implemented in code: the workspace has a fail-closed manifest-backed registry, local deterministic tool implementations, and one shipped workflow path that seeds and consumes `ToolCall` artifacts end-to-end; the next gaps are model-backed tool adapters and durable event-plane persistence
...
- reads `pending_tool` and `pending_tool_params` from workflow artifacts
